...
export class TicketsServiceService implements OnModuleInit {
  ...
  async findOne(id: string, userId: string) {
    const [foundTickets] = await this.dbService.db
      .select({
        id: tickets.id,
  ...
  async purchase(purchaseDto: PurchaseTicketDto, userId: string) {
    ...
    const soldTicket = await this.dbService.db
      .select({
        total: sql<number>`COALESC(count(SUM(${tickets.quantity})), 0)`,
      })
      .from(tickets)
      .where(
    ...
    const currentSold = Number(soldTicket[0]?.total);
  ...
  async findEventTickets(eventId: string, organizerId: string) {
    ...
    if (event.organizerId !== organizerId) {
      throw new ForbiddenException(
        'You are not authorized to view tickets of this event',
      );
    }
