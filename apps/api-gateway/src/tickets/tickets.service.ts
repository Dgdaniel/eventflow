...
export class TicketsService {
  ...
  async purchaseTicket(purchaseDto: PurchaseTicketDto, userId: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.post(`${this.ticketService}/purchase`, purchaseDto, {
          headers: {
            'x-user-id': userId,
          },
        }),
    ...
    try {
      const response: AxiosResponse = await firstValueFrom(
        this.httpService.post(
          `${this.ticketService}/${ticketId}/cancel`,
          {},
          {
            headers: {
              'x-user-id': userId,
  ...
  async validateTicket(ticketId: string, userId: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.post(
          `${this.ticketService}/${ticketId}/validate`,
          {},
          {
            headers: {
              'x-user-id': userId,
  ...
  async findEventTickets(eventId: string, userId: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.ticketService}/events/${eventId}`, {
          headers: {
            'x-user-id': userId,
          },
        }),
  ...
  async findMyTickets(userId: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.ticketService}/my-tickets`, {
          headers: {
            'x-user-id': userId,
          },
        }),
  ...
  async findOne(ticketId: string, userId: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.ticketService}/${ticketId}`, {
          headers: {
            'x-user-id': userId,
          },
        }),
